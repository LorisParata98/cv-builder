...
async function deeplBatch(texts, targetLang, apiKey) {
  ...
  body.append("auth_key", apiKey.trim());
...
export async function translateCV(cvData, targetLang, apiKey) {
  ...
  // Personal
  add(result.personal?.summary, (v) => { result.personal.summary = v; });

  // Experience
  result.experience?.forEach((exp, ei) => {
    add(exp.role, (v) => { result.experience[ei].role = v; });
    exp.bullets?.forEach((b, bi) => {
      add(b, (v) => { result.experience[ei].bullets[bi] = v; });
    });
  });

  // Education
  result.education?.forEach((edu, ei) => {
    add(edu.degree, (v) => { result.education[ei].degree = v; });
    add(edu.field,  (v) => { result.education[ei].field  = v; });
    add(edu.thesis, (v) => { result.education[ei].thesis = v; });
  });

  // Certifications
  result.certifications?.forEach((c, ci) => {
    add(c, (v) => { result.certifications[ci] = v; });
  });

  // Projects
  result.projects?.forEach((p, pi) => {
    add(p, (v) => { result.projects[pi] = v; });
  });

...
function AtsHintsPanel({ template }) {
  ...
  const label =
    { tech: "Tech", manager: "Manager", designer: "Designer" }[template] ||
    "Tech";
    ...
    <div className="mb-3 border border-green-200 rounded-lg overflow-hidden">
      ...
      >
        <span className="text-xs font-semibold text-green-800 flex items-center gap-2">
          ...
          Keyword ATS consigliate — profilo {label}
        </span>
      ...
      {open && (
        <div className="px-3 py-3 bg-white">
          <p className="text-xs text-gray-500 mb-2">
            Assicurati che queste keyword compaiano nelle tue skill o nei bullet
            delle esperienze.
          </p>
          <div className="flex flex-wrap gap-1">
            {keywords.map((kw, i) => (
              <span
                key={i}
                className="text-xs bg-green-50 border border-green-300 text-green-800 px-2 py-0.5 rounded-full"
              >
                {kw}
              </span>
            ))}
        ...
        <div className="mt-2 pt-2 border-t border-gray-100">
          ...
          <div className="flex flex-wrap gap-1 mb-1">
            {(tag.atsKeywords || []).map((kw, ki) => (
              <span
                key={ki}
                className="inline-flex items-center gap-1 text-xs bg-gray-100 border border-gray-300 rounded px-2 py-0.5"
              >
                {kw}
                <button
                  onClick={() =>
                    onUpdate({
                      atsKeywords: (tag.atsKeywords || []).filter(
                        (_, i) => i !== ki,
                      ),
                    })
                  }
          ...
          <p className="text-xs text-gray-400 mt-1">
            Invisibili nel PDF · usate dai parser ATS
          </p>
        </div>
      )}
    </div>
    ...
    >
      <div className="flex items-center gap-2 bg-gray-50 px-3 py-2.5 border-b border-gray-200">
        ...
        <input
          ...
          onChange={(e) =>
            onUpdateCategory(index, { category: e.target.value })
          }
...
export function SkillsForm() {
  ...
  return (
    <SectionCard
      title="Competenze"
      icon={<Wrench size={15} weight="duotone" />}
    >

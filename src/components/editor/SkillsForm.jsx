...
function TagEditor({ tag, onUpdate, onRemove }) {
  ...
  const handleAutoGenerate = () => {
    const keywords = generateAtsKeywords(tag.label);
    onUpdate({ atsKeywords: keywords });
  };

  return (
    <div className="border border-gray-200 rounded p-2 mb-2 bg-white">
      {/* Riga 1: label + rimuovi */}
          ...
          <div className="flex flex-wrap gap-1 mb-1">
            {(tag.atsKeywords || []).map((kw, ki) => (
              <span key={ki} className="inline-flex items-center gap-1 text-xs bg-gray-100 border border-gray-300 rounded px-2 py-0.5">
                ...
                <button
                  onClick={() => {
                    const updated = (tag.atsKeywords || []).filter((_, i) => i !== ki);
                    onUpdate({ atsKeywords: updated });
                  }}
...
function CategoryEditor({ category, index, onUpdateCategory, onRemoveCategory,
  onAddTag, onRemoveTag, onUpdateTag }) {
  return (
      ...
      <div className="p-2">
        {category.tags.map((tag, ti) => (
          <TagEditor
            key={ti}
            tag={tag}
            onUpdate={(updates) => onUpdateTag(index, ti, updates)}

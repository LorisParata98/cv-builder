...
export function ProjectsForm() {
  ...
  return (
    <SectionCard title={L.title} icon={<Rocket size={15} weight="duotone" />}>
      <p className="text-xs text-gray-400 mb-4">{L.hint}</p>
      <div className="space-y-2.5">
        {projects.map((proj, i) => (
          ...
          >
            ...
            />
            {projects.length > 1 && (
              <button onClick={() => removeProject(i)} className="text-gray-300 hover:text-red-400 mt-1.5 text-sm flex-shrink-0">✕</button>
            )}
          </div>
        ))}
      </div>

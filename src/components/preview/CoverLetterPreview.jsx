...
function getTemplateColors(template, designerPalette, customColors = {}) {
  ...
  if (template === "designer") {
    const p =
      DESIGNER_PALETTES_COLORS[designerPalette] ||
      DESIGNER_PALETTES_COLORS["noir-gold"];
    return {
...
function ZoomControls({ zoom, onZoom, onReset }) {
  ...
  const btnBase =
    "w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold transition-colors select-none";
  return (
    <div
      style={{
        position: "absolute",
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, padding: "36px 48px 40px" }}>
        ...
        {cl.hiringManager ? (
          <div
            style={{ marginBottom: "18px", fontWeight: 500, color: "#1e293b" }}
          >
        ...
        {hasContent ? (
          ...
          >
            ...
            <div style={{ fontWeight: 600, marginBottom: "6px" }}>
              Testo non ancora inserito
            </div>
            <div>
              Usa il form a sinistra per generare il prompt AI,
              <br />
              poi incolla il testo nel campo "Corpo della lettera".
            </div>
          </div>
        )}
...
export function CoverLetterPreview() {
  ...
  const [zoomIndex, setZoomIndex] = useState(ZOOM_STEPS.indexOf(0.9));
